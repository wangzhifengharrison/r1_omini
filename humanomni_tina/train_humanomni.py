...
from humanomni.mm_utils import tokenizer_multimodal_token, process_image, process_video, read_video_patch, \
    process_audio, frame_sample
...
class ModelArguments:
    # LLM Arguments
    model_type: Optional[str] = field(default="HumanOmni_qwen2",
                                      metadata={"help": "Model type selected in the list: " + ", ".join(VLLMs.keys())})
    ...
    for i, source in enumerate(sources):
        ...
        for idx, sentence in enumerate(source):
            if idx % 2 == 1:
                ...
                instruction = tokenizer.apply_chat_template(message + tmp_message[:1], tokenize=False,
                                                            add_generation_prompt=True)
                conversation = tokenizer.apply_chat_template(message + tmp_message, tokenize=False,
                                                             add_generation_prompt=False)

                instruction_len = len(
                    tokenizer_multimodal_token(instruction, tokenizer, modal_token, return_tensors='pt'))
                conversation_len = len(
                    tokenizer_multimodal_token(conversation, tokenizer, modal_token, return_tensors='pt'))
        ...
        for sentence in source:
            for DEFAULT_TOKEN in MODAL_INDEX_MAP.keys():
                ...
                if DEFAULT_TOKEN in sentence['value']:
                    ...
                    sentence['value'] = sentence['value'].replace(f"{DEFAULT_TOKEN}", '').replace(DEFAULT_TOKEN,
                                                                                                  '').strip()
                    sentence['value'] = replace_token + '\n' + sentence['value']
                    sentence['value'] = sentence['value'].strip()
                if data_args.mm_use_x_start_end:
                    new_replace_token = DEFAULT_X_START_TOKEN[X.upper()] + replace_token + DEFAULT_X_END_TOKEN[
                        X.upper()]
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            try:
                video = process_video(video_file, video_processor, aspect_ratio=self.data_args.image_aspect_ratio,
            ...
            if audio_processor is not None:
                try:
                    ...
                    audio = audio_processor(audio, sampling_rate=audio_sample_rate, return_tensors='pt')[
                        'input_features']
                except Exception as e:
                    traceback.print_exc()
                    audio = None
            ...
            try:
                modal_token = '<audio>'
                audio, sampling_rate = torchaudio.load(self.list_data_dict[i]['audio'], normalize=True,
                                                       channels_first=True)  # TODO: Check norm
                audio = torchaudio.transforms.Resample(orig_freq=sampling_rate, new_freq=audio_processor.sampling_rate)(
                    audio)
                if len(audio.shape) > 1 and sampling_rate != -1:
                    audio = audio.mean(dim=0)
                audio_processor = getattr(self.data_args, "audio_processor", None)
                audio = audio_processor(audio, sampling_rate=audio_processor.sampling_rate, return_tensors='pt')[
                    'input_features']
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
            except Exception as e:
                traceback.print_exc()
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        inputs_bert = bert_tokenizer(prompts_bert, return_tensors='pt', padding=True, truncation=True,
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None or model_args.audio_tower is not None:
        if model_args.vision_tower is not None:
            ...
            data_args.video_processor = vision_tower.video_processor if hasattr(vision_tower,
                                                                                "video_processor") else vision_tower.image_processor
