...
class HumanOmniMetaModel:
    ...
    def __init__(self, config):
        ...
        num_branches = 3
        bert_model = "/home/qixuan/Documents/R1-Omni/bert_base_uncased"
...
class HumanOmniMetaForCausalLM(ABC):
    ...
    def initialize_vision_tokenizer(self, model_args, tokenizer=None):
        """初始化视觉tokenizer，在训练过程中需要使用此方法"""
        vision_tower = self.get_vision_tower()
        if vision_tower is not None and hasattr(vision_tower, 'image_processor'):
            vision_tower.image_processor.tokenizer = tokenizer
            if hasattr(vision_tower, 'video_processor'):
                vision_tower.video_processor.tokenizer = tokenizer
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if labels is not None:
                ...
                cur_new_labels = []
                # 修改断言检查，打印形状信息并尝试调整形状
                if cur_labels.shape != cur_input_ids.shape:
                    print(f"警告: 标签形状与输入ID形状不匹配!")
                    print(f"标签形状: {cur_labels.shape}, 输入ID形状: {cur_input_ids.shape}")
                    
                    # 尝试调整标签形状以匹配输入ID
                    if cur_labels.shape[0] < cur_input_ids.shape[0]:
                        # 如果标签比输入短，扩展标签(用IGNORE_INDEX填充)
                        padding = torch.full((cur_input_ids.shape[0] - cur_labels.shape[0],), 
                                            IGNORE_INDEX, 
                                            device=cur_labels.device, 
                                            dtype=cur_labels.dtype)
                        cur_labels = torch.cat([cur_labels, padding], dim=0)
                        print(f"标签已扩展到: {cur_labels.shape}")
                    elif cur_labels.shape[0] > cur_input_ids.shape[0]:
                        # 如果标签比输入长，截断标签
                        cur_labels = cur_labels[:cur_input_ids.shape[0]]
                        print(f"标签已截断到: {cur_labels.shape}")
                    
                    # 再次检查形状是否匹配
                    if cur_labels.shape != cur_input_ids.shape:
                        raise AssertionError(f"无法调整标签形状以匹配输入ID形状: {cur_labels.shape} vs {cur_input_ids.shape}")
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            ...
            if attention_mask is not None:
                ...
                if attention_mask.shape != new_labels.shape:
                    print(f"警告: 注意力掩码形状与标签形状不匹配!")
                    print(f"注意力掩码形状: {attention_mask.shape}, 标签形状: {new_labels.shape}")
                    
                    # 尝试调整注意力掩码以匹配标签形状
                    if attention_mask.shape[0] == new_labels.shape[0]:
                        # 批次大小相同，只需调整序列长度
                        if attention_mask.shape[1] < new_labels.shape[1]:
                            # 如果注意力掩码比标签短，扩展掩码(用False填充)
                            padding = torch.full((attention_mask.shape[0], new_labels.shape[1] - attention_mask.shape[1]), 
                                                False, 
                                                device=attention_mask.device, 
                                                dtype=attention_mask.dtype)
                            attention_mask = torch.cat([attention_mask, padding], dim=1)
                            print(f"注意力掩码已扩展到: {attention_mask.shape}")
                        elif attention_mask.shape[1] > new_labels.shape[1]:
                            # 如果注意力掩码比标签长，截断掩码
                            attention_mask = attention_mask[:, :new_labels.shape[1]]
                            print(f"注意力掩码已截断到: {attention_mask.shape}")
                    
                    # 再次检查形状是否匹配
                    if attention_mask.shape != new_labels.shape:
                        print(f"无法自动调整注意力掩码形状以匹配标签形状: {attention_mask.shape} vs {new_labels.shape}")
                        # 不抛出异常，继续执行
        else:
            ...
            if attention_mask is not None:
                ...
                if attention_mask.shape != new_input_embeds.shape[:2]:
                    print(f"警告: 注意力掩码形状与输入嵌入形状不匹配!")
                    print(f"注意力掩码形状: {attention_mask.shape}, 输入嵌入形状前两维: {new_input_embeds.shape[:2]}")
                    
                    # 尝试调整注意力掩码以匹配输入嵌入形状
                    if attention_mask.shape[0] == new_input_embeds.shape[0]:
                        # 批次大小相同，调整序列长度
                        if attention_mask.shape[1] < new_input_embeds.shape[1]:
                            # 如果注意力掩码比输入嵌入短，扩展掩码(用True填充前面部分，这通常是合理的)
                            padding = torch.full((attention_mask.shape[0], new_input_embeds.shape[1] - attention_mask.shape[1]), 
                                                True, 
                                                device=attention_mask.device, 
                                                dtype=attention_mask.dtype)
                            attention_mask = torch.cat([padding, attention_mask], dim=1)
                            print(f"注意力掩码已扩展到: {attention_mask.shape}")
                        elif attention_mask.shape[1] > new_input_embeds.shape[1]:
                            # 如果注意力掩码比输入嵌入长，截断掩码
                    ...
                    # 再次检查形状是否匹配
                    if attention_mask.shape != new_input_embeds.shape[:2]:
                        print(f"无法自动调整注意力掩码形状以匹配输入嵌入形状: {attention_mask.shape} vs {new_input_embeds.shape[:2]}")
                        # 不抛出异常，继续执行
