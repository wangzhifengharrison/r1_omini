...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        for index, row in df.iterrows():
            ...
            instruct = (
                "As an emotional recognition expert, which emotion is most obvious? "
                "Output thinking in <think></think> and emotion in <answer></answer>"
            )
            ...
            target_answer = f"<think>I can see this person's facial expressions and body language clearly showing {row['label']} emotion.</think><answer>{row['label']}</answer>"
            
            # 为BERT模型准备输入
            question = "Which emotion is most obvious in this video?"
            question_input = bert_tokenizer([question], return_tensors='pt', padding=True, truncation=True, add_special_tokens=True)
